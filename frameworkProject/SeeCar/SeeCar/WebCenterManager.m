...
#include <sys/xattr.h>
#import <WebIntercept/MyURLProtocol.h>
...
- (void)removeDownLoadCar:(NSString *)carId status:(NSString *)status{
    ...
    if ([status isEqualToString:@"1"]) {
        [downLoadModel.versions setObject:@"" forKey:carId];
    }
...
- (void)downLoadError:(NSNotification *)noification{
    ...
    if (dic) {
        ...
        [downLoadModel.downStatus setObject:@"1" forKey:carId];
        [[DownLoadCenterManager shareInstance].downLoadStatus setObject:@"1" forKey:carId];
        dispatch_async(dispatch_get_global_queue(0, 0),^{
            [NSKeyedArchiver archiveRootObject:downLoadModel toFile:[DownLoadCacheModel systemMsgCachePath]];
        });
    }
...
- (void)nextDownLoadCar:(NSString *)carId{
    ...
    [downLoadModel.downStatus setObject:@"4" forKey:carId];
    [[DownLoadCenterManager shareInstance].downLoadStatus setObject:@"4" forKey:carId];
    NSString *tempVersion             = downLoadModel.tempVersions[carId];
    if (tempVersion && tempVersion.length) {
         [downLoadModel.versions setObject:tempVersion forKey:carId];
    }
