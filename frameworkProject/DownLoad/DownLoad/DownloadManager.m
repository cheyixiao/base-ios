...
//    [WebSourceManager shareInstance].currentArr = downloadModels;
    ...
    [[DownLoadCenterManager shareInstance].downLoadHashDic removeAllObjects];
    for (WebDownloadModel *model in downloadModels) {
        if (model && model.md5Name && model.md5Name.length) {
            [[DownLoadCenterManager shareInstance].downLoadHashDic setObject:model.md5Name forKey:model.urlString];
        }
    }
}
/**
...
//        [[NSNotificationCenter defaultCenter] postNotificationName:@"webSourceDownLoadError" object:nil userInfo:@{@"carId":[DownloadManager sharedManager].downLoadCenterManager.currentCarID}];
...
- (void)downLoadCompleted:(WebDownloadModel *)downloadModel{
