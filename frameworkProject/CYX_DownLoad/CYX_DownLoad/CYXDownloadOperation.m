...
@interface CYXDownloadOperation (){
...
@implementation CYXDownloadOperation
...
- (instancetype)initWithDownloadModel:(CYXWebDownloadModel *)downloadModel andSession:(NSURLSession *)session{
    ...
    if (self) {
        ...
        self.downloadModel.status = kCYXDownloadStatus_Waiting;
    }
    return self;
}
...
- (void)resume{
    ...
    //等待中的任务交给队列调度
    if (self.downloadModel.status == kCYXDownloadStatus_Waiting)
        return;
    ...
    if ([keyPath isEqualToString:@"state"]) {
        ...
        switch (newState) {
            case NSURLSessionTaskStateSuspended:
                self.downloadModel.status = kCYXDownloadStatus_Suspended;
                //为进行任务管理 暂停任务后 直接取消
                [self cancel];
                break;
            case NSURLSessionTaskStateCompleted:{
                if (self.downloadModel.isFinished) {
                    self.downloadModel.status = kCYXDownloadStatus_Completed;
                    [self cancel];
                }else{
                    if (self.downloadModel.status == kCYXDownloadStatus_Suspended) {
                    }else{// 下载失败
                        self.downloadModel.status = kCYXDownloadStatus_Failed;
                        [self resetDownLoad:self.downloadModel];
                    }
                }
            }break;
            case NSURLSessionTaskStateRunning:
                self.downloadModel.status = kCYXDownloadStatus_Running;
                break;
...
- (void)resetDownLoad:(CYXWebDownloadModel *)model{
    if (!model || !model.urlString || !self) {
        return;
    }
    if (![[WebSourceManager shareInstance].downLoadFailUrls containsObject:model.urlString] && [WebSourceManager shareInstance].cutNet == NO) {
        
        [[WebSourceManager shareInstance].downLoadFailUrls addObject:model.urlString];
    }
...
- (void)cancel{
    ...
    BOOL isWaiting = self.downloadModel.status == kCYXDownloadStatus_Waiting;
