...
@implementation CYXWebDownloadModel
...
- (NSString *)destinationPath {
    ...
    NSString *patientPhotoFolder = [[CYXCachesDirectory stringByAppendingPathComponent:self.carID] stringByAppendingPathComponent:[self.fileName stringByDeletingLastPathComponent]];
...
- (void)setStatus:(CYXDownloadStatus)status{
...
- (void)setStatusTextWith:(CYXDownloadStatus)status{
    _status = status;
    
    switch (status) {
        case kCYXDownloadStatus_Running:
            self.statusText = @"正在下载";
            break;
        case kCYXDownloadStatus_Suspended:
            self.statusText = @"暂停下载";
            break;
        case kCYXDownloadStatus_Failed:
            self.statusText = @"下载失败";
            break;
        case kCYXDownloadStatus_Cancel:
            self.statusText = @"取消下载";
            break;
        case kCYXDownloadStatus_Waiting:
            self.statusText = @"等待下载";
            break;
        case kCYXDownloadStatus_Completed:
