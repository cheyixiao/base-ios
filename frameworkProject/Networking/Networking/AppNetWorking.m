...
#import <BaseFramework/NSString+Extension.h>
#import "AFNetworking.h"
#import "AFHTTPSessionManager.h"
#import <CheYiXiaoBase/AppFrameworkTool.h>
#import "MBProgressHUD.h"
#import <CheYiXiaoBase/BaseAppDelegate.h>
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            dispatch_async(dispatch_get_main_queue(), ^{
                if( task.state != NSURLSessionTaskStateCanceling ){
                    ...
                    if (code == 302||code == 403) {
                        [[UserDefaults shareInstance] clearLoginInfo];
                        [(BaseAppDelegate *)[[UIApplication sharedApplication] delegate] chooseViewController];
                        [AppFrameworkTool showAlertWithMessage:@"账号已在其他设备上登录，请重新登录"];
                    }else{
                        if( success ) {
                            success(result);
                        }
